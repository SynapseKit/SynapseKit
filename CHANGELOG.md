...
- **`import synapsekit` no longer requires the `httpx` extra** — `NotionTool` (pulled in unconditionally by `synapsekit.agents.tools`) did a module-level `import httpx`, so a bare `pip install synapsekit` (no extras) failed to import the whole package with `ModuleNotFoundError: No module named 'httpx'`. `httpx` is now imported lazily inside the methods that use it (matching the other HTTP tools), and a `tests/test_core_import_without_extras.py` guard imports the package in a fresh interpreter with every optional dependency blocked, so this bug class can't regress. Surfaced by the new core Docker image build.
