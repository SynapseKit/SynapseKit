...
- **Provider cost-arbitrage router & budget governance** (#893) — `CostQualityRouter` gained provider-aware pricing (`ModelPricing`/`PricingTable` with cached-input rates, latency SLAs, quality metadata, regional overrides, and pluggable refresh sources), quality/latency-aware arbitrage, tenant/API-key budget enforcement via `BudgetLedger` (atomic reservations, settlement caps, spend attribution), fallback-chain composition, and optional carbon accounting. Adds `synapsekit_budget_remaining_usd` and `synapsekit_arbitrage_savings_usd` Prometheus gauges, and a `cost-arbitrage-gate` CI job asserting ≥40% savings at equal quality over a deterministic 100-request replay. Once a `budget_ledger` is configured, routing to a model missing from the pricing table now hard-fails rather than silently costing $0 — register custom/self-hosted pricing with `pricing_table.register(...)` before enabling budgets for them. Post-merge review fixed three bugs: `BudgetLedger.record_spend()` wrote spend under raw (unstringified) tenant/API-key identifiers while every budget check read via `str(...)`, silently bypassing the cap for non-string identifiers; `CostQualityRouter.stream()` swallowed total-candidate-failure exceptions that `generate()` correctly raised, so a caller iterating `stream()` couldn't tell a genuinely empty response from every candidate crashing; and a post-call budget-settlement overage could discard an already-successful LLM response — `commit()` now supports `force=True` to record real spend without re-checking the cap, and the router logs the overage instead of throwing away a response the caller already paid for. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
...
- **Bumped 4 dependencies off known-vulnerable versions; dismissed the 3 `chromadb` alerts that were triaged but never actually closed.** An OSV audit of the full resolved graph (562 packages) flagged 10 open Dependabot alerts beyond the already-triaged `chromadb` ones: `anyio` floored to `>=4.14.2` (GHSA-82r6-8w77-94w6 critical — TLS certificate spoofing via IDNA 2003 hostname encoding — plus a medium stderr-drain hang); `soupsieve` floored to `>=2.9.0` (two medium ReDoS advisories in selector parsing, GHSA-gjv8-xp57-g29c / GHSA-j934-xhv5-fg8f); and the `mcp` extra's `httpx2`/`httpcore2` transitive deps floored to `>=2.12.0`/`>=2.10.0` respectively (one high — secure WebSocket traffic sent unencrypted through SOCKS proxies, GHSA-7mj9-2mp8-4m2p — plus five medium/high advisories covering decompression-amplification memory blowup, header-smuggling, and CPU DoS via SSE line buffering). All four floors added to `[tool.uv] constraint-dependencies`. Also actually dismissed the 3 `chromadb` Dependabot alerts (#127–129) per the risk-acceptance rationale below, which had been documented in PR #943 but never applied via the API. Post-fix OSV re-audit: 0 known vulnerabilities outside `chromadb` (no fix available) and an unrelated, likely-stale `couchbase` server-side advisory (PYSEC-2023-235, not flagged by Dependabot, not a client-library issue) that's tracked separately. No SynapseKit public API changed.
