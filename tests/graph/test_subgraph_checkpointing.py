...
async def test_subgraph_gets_own_checkpoint():
    ...
    result = await parent.compile().run({"value": 5}, checkpointer=cp, graph_id="parent1")
...
async def test_nested_subgraphs_get_nested_scopes():
    ...
    mid_graph.add_node("inner_sub", subgraph_node(inner_graph.compile(), name="inner"))
    mid_graph.set_entry_point("inner_sub").set_finish_point("inner_sub")

    outer_graph = StateGraph()
    outer_graph.add_node("mid_sub", subgraph_node(mid_graph.compile(), name="middle"))
    outer_graph.set_entry_point("mid_sub").set_finish_point("mid_sub")

    cp = InMemoryCheckpointer()
    result = await outer_graph.compile().run({}, checkpointer=cp, graph_id="root")
...
async def test_subgraph_checkpoint_with_on_error_skip():
    ...
    parent = StateGraph()
    parent.add_node("sub", subgraph_node(sub.compile(), name="skipper", on_error="skip"))
    parent.set_entry_point("sub").set_finish_point("sub")
