...
def test_load_sheet_data(mock_build, mock_creds, mock_exists):
...
@patch("google.oauth2.service_account.Credentials.from_service_account_file")
@patch("googleapiclient.discovery.build")
def test_load_default_sheet(mock_build, mock_creds, mock_exists):
    mock_service = MagicMock()
    mock_build.return_value = mock_service
    mock_sheets_api = mock_service.spreadsheets.return_value
...
@patch("google.oauth2.service_account.Credentials.from_service_account_file")
@patch("googleapiclient.discovery.build")
def test_load_empty_sheet(mock_build, mock_creds, mock_exists):
    mock_service = MagicMock()
    mock_build.return_value = mock_service
    mock_sheets_api = mock_service.spreadsheets.return_value
