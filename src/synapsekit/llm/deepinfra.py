"""DeepInfra OpenAI-compatible LLM provider."""
...
class DeepInfraLLM(BaseLLM):
    """DeepInfra chat completions over its OpenAI-compatible API."""

    def __init__(self, config: LLMConfig, base_url: str | None = None) -> None:
        super().__init__(config)
        self._base_url = base_url or _DEEPINFRA_BASE_URL
    ...
    def _get_client(self) -> Any:
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "openai package required: pip install synapsekit[deepinfra]"
                ) from None
