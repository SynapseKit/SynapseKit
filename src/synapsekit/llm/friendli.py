"""FriendliAI OpenAI-compatible LLM provider."""
...
_FRIENDLI_BASE_URL = "https://api.friendli.ai/serverless/v1"


class FriendliLLM(BaseLLM):
    """FriendliAI chat completions over its OpenAI-compatible API."""

    def __init__(self, config: LLMConfig, base_url: str | None = None) -> None:
        super().__init__(config)
        self._base_url = base_url or _FRIENDLI_BASE_URL
    ...
    def _get_client(self) -> Any:
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "openai package required: pip install synapsekit[friendli]"
                ) from None
