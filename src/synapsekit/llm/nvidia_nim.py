"""NVIDIA NIM REST provider.

NVIDIA-hosted NIM exposes an OpenAI-shaped chat-completions endpoint, but this
provider intentionally uses the native async HTTP API rather than an OpenAI
SDK so that self-hosted, unauthenticated NIM deployments work as well.
"""
...
def _load_httpx() -> Any:
    try:
        ...
        raise ImportError("httpx required: pip install synapsekit[nvidia-nim]") from None
    return httpx
...
def _event_from_line(line: str) -> tuple[dict[str, Any] | None, bool]:
    """Decode one SSE line, also accepting a one-line non-SSE JSON response."""
    stripped = line.strip()
...
class NvidiaNIMLLM(BaseLLM):
    """NVIDIA NIM chat completions over ``httpx.AsyncClient``."""
    ...
    async def _call_with_tools_impl(
        ...
        """Call an OpenAI-compatible NIM endpoint with native tools."""
        ...
        response = await self._get_client().post(
            self._endpoint(), headers=self._headers(), json=tool_payload
        )
        response.raise_for_status()
        data = response.json()
