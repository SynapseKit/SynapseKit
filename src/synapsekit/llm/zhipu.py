...
class ZhipuLLM(BaseLLM):
    """Zhipu AI (GLM) LLM provider with async streaming.

    Uses the OpenAI-compatible API. Models include ``glm-4``,
    ``glm-4-flash``, ``glm-3-turbo``.
    """

    def __init__(self, config: LLMConfig, base_url: str | None = None) -> None:
        super().__init__(config)
        self._base_url = base_url or _ZHIPU_BASE_URL
