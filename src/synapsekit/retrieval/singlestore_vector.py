"""SingleStore vector-search adapter."""
...
class SingleStoreVectorStore(RemoteVectorStoreSupport, VectorStore):
    """Async adapter using SingleStore's SQL vector functions."""
    ...
    def __init__(
        ...
        if connection is None:
            try:
                ...
                raise ImportError(
                    "singlestoredb required: pip install synapsekit[singlestore]"
                ) from None
            if connection_string:
                connection = singlestoredb.connect(connection_string)
            else:
    ...
    def _search_sync(
        ...
        try:
            rows = self._execute(
                f"SELECT text, metadata, DOT_PRODUCT(embedding, %s) AS score "
                f"FROM {self._table_name} ORDER BY score DESC LIMIT %s",
                (json.dumps(vector), top_k * 10 if metadata_filter else top_k),
            )
