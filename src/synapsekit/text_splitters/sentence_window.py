...
class SentenceWindowSplitter(BaseSplitter):
    ...
    def _create_windows(self, sentences: list[str]) -> list[str]:
        """
        Create sentence windows from a list of sentences.

        Args:
            sentences: List of sentences to create windows from.

        Returns:
            List of text chunks, each containing a target sentence with context.
        """
        windows: list[str] = []
        ...
        for idx in range(len(sentences)):
            ...
            # Get sentences in the window
            window_sentences = sentences[start:end]

            # Join them into a single chunk
